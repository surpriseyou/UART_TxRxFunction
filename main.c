...
uint8_t g_u8RecData1[RXBUFSIZE]  = {0};

volatile uint32_t g_u32comRbytes = 0;
volatile uint32_t g_u32comRhead  = 0;
volatile uint32_t g_u32comRtail  = 0;
volatile int32_t g_bWait         = TRUE;
volatile uint32_t g_u32comRbytes1 = 0;
volatile uint32_t g_u32comRhead1  = 0;
volatile uint32_t g_u32comRtail1  = 0;
...
void UART_TEST_HANDLE()
{
    ...
    if(u32IntSts & UART_INTSTS_RDAINT_Msk)
    {
        ...
        while(UART_IS_RX_READY(UART0))
        {
            ...
            u8InChar = UART_READ(UART0);    // ==>    u8InChar = ((uart)->DAT)
...
void UART1_TEST_HANDLE()
{
    uint8_t u8InChar1 = 0xFF;
    uint32_t u32IntSts1 = UART1->INTSTS;

    if(u32IntSts1 & UART_INTSTS_RDAINT_Msk)
    {
        printf("\nInput:");
        ...
        {
            /* Get the character from UART Buffer */
            u8InChar1 = UART_READ(UART1);    // ==>    u8InChar = ((uart)->DAT)
            ...
            /* Check if buffer full */
            if(g_u32comRbytes1 < RXBUFSIZE)
            {
                /* Enqueue the character */
                g_u8RecData1[g_u32comRtail] = u8InChar1;
                g_u32comRtail1 = (g_u32comRtail1 == (RXBUFSIZE - 1)) ? 0 : (g_u32comRtail1 + 1);
                g_u32comRbytes1++;
            }
        }
        printf("\nTransmission Test:");
    }
    if(u32IntSts1 & UART_INTSTS_THREINT_Msk)
    {
        uint16_t tmp;
        tmp = g_u32comRtail1;
        if(g_u32comRhead1 != tmp)
        {
            u8InChar1 = g_u8RecData1[g_u32comRhead];
            while(UART_IS_TX_FULL(UART1));  /* Wait Tx is not full to transmit data */
            UART_WRITE(UART0, u8InChar1);
            g_u32comRhead1 = (g_u32comRhead1 == (RXBUFSIZE - 1)) ? 0 : (g_u32comRhead1 + 1);
            g_u32comRbytes1--;
        }
    }
}
